//! Sui ITS transaction submission.
//!
...
use crate::sui::{
    InterchainTransferRequest, SuiClient, SuiItsContractsConfig, SuiWallet, read_sui_chain_config,
    read_sui_its_config, send_its_interchain_transfer,
};
use crate::ui;
...
pub(super) async fn prepare_source(args: &LoadTestArgs) -> Result<PreparedSuiIts> {
    ...
    let (default_rpc, _) = read_sui_chain_config(&args.config, source_chain).await?;
    ...
    let contracts = read_sui_its_config(&args.config, source_chain).await?;
    ui::address(
...
impl TransactionSubmitter for ItsSuiSubmitter {
    ...
    async fn submit(&self, (): Self::Job) -> TxMetrics {
        ...
        let result = send_its_interchain_transfer(InterchainTransferRequest {
            client: &self.client,
            wallet: &self.wallet,
