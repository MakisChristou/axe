//! Stellar ITS transaction submission.
    ...
    V: RemoteDeploymentVerifier + Sync,
{
    ...
    let source_axelar_id =
        super::axelar_id_for_chain(&request.config, &request.source_chain).await?;
    remote_verifier
    ...
    super::save_its_cache(&request.source_chain, &request.destination_chain, &cache).await?;
    ...
    V: RemoteDeploymentVerifier + Sync,
{
    ...
    let cache = super::read_its_cache(&request.source_chain, &request.destination_chain).await;
...
fn parse_token_id(value: &str) -> StdResult<[u8; 32], TokenIdParseError> {
