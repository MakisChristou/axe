//! Load test configuration and cache resolution.
...
pub(crate) async fn read_cache(axelar_id: &str) -> GmpCache {
    read_json_cache(&cache_path(axelar_id)).await
}

pub(crate) async fn save_cache(axelar_id: &str, cache: &GmpCache) -> Result<()> {
    let path = cache_path(axelar_id);
    if let Some(parent) = path.parent() {
        fs::create_dir_all(parent).await?;
    }
    fs::write(&path, serde_json::to_string_pretty(cache)?).await?;
    Ok(())
}
...
pub(crate) async fn resolve_from_config(
    config: &PathBuf,
...
pub(crate) async fn read_its_cache(src: &str, dst: &str) -> ItsCache {
    read_json_cache(&its_cache_path(src, dst)).await
}

pub(crate) async fn save_its_cache(src: &str, dst: &str, cache: &ItsCache) -> Result<()> {
...
pub(crate) async fn find_cached_salt(token_id: &str) -> Option<String> {
...
pub(crate) fn detect_network_from_config(config: &Path) -> Option<Network> {
...
mod tests {
    ...
    #[tokio::test]
    async fn missing_cache_is_an_empty_object() {
        let path = cache_fixture("missing-cache");

        let cache: ItsCache = read_json_cache(&path).await;
    ...
    #[tokio::test]
    async fn malformed_cache_preserves_empty_cache_fallback() {
        let path = cache_fixture("malformed-cache");
        fs::write(&path, "{").await.unwrap();

        let cache = read_json_cache::<ItsCache>(&path).await;
    ...
    #[tokio::test]
    async fn valid_cache_is_parsed() {
        let path = cache_fixture("valid-cache");
        fs::write(&path, r#"{"tokenId":"0x01","futureField":7}"#)
            .await
            .unwrap();
