//! Typed projections of the Axelar configuration needed by verification.
...
pub(super) fn load_gmp_axelar_config(
    config: &Path,
    source_chain: &str,
...
pub(super) fn load_its_axelar_config(config: &Path, source_chain: &str) -> Result<ItsAxelarConfig> {
    let cfg = ChainsConfig::load(config)?;
    let (lcd, _, _, _) = cfg.axelar.cosmos_tx_params()?;
    let voting_verifier = cfg
        .axelar
