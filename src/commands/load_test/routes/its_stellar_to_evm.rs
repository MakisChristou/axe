...
impl RemoteDeploymentVerifier for EvmRemoteDeployment {
    ...
    fn after_remote_deploy(&self, source_chain: &str, token_id: [u8; 32]) {
        super::helpers::hint_persist_axe_token(source_chain, &FixedBytes::from(token_id));
    }
}
...
async fn init_stellar_setup(args: &LoadTestArgs, src: &str) -> Result<StellarSetup> {
    ...
    let network_type = super::read_stellar_network_type(&args.config, src).await?;
    ...
    let stellar_gateway_addr =
        super::read_stellar_contract_address(&args.config, src, ChainContract::AxelarGateway)
            .await?;
    let stellar_xlm_addr = super::read_stellar_token_address(&args.config, src).await?;
...
fn resolve_evm_targets(cfg: &ChainsConfig, dest: &str) -> Result<EvmTargets> {
    ...
    let evm_its_addr: Address = dest_cfg
