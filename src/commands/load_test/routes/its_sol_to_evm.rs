...
use crate::solana;
use crate::types::{DEAD_ADDRESS, LOAD_TEST_SOL_SPEC, Network};
use crate::ui;
...
fn mint_decimals(rpc_client: &RpcClient, mint: &Pubkey) -> u8 {
    rpc_client
...
/// `--gas-value`.
fn default_gas_value(network: Network) -> u64 {
    match network {
...
pub async fn run(args: LoadTestArgs, sizing: RunSizing) -> eyre::Result<()> {
    ...
    let main_keypair = init_solana_client_and_main_keypair(
        &args.source_rpc,
    ...
    let prepared =
        spawn_blocking(move || prepare_source_accounts(&source_rpc, &main_keypair, mint, sizing))
            .await??;
    ...
    let transfer = ItsTransferSpec {
        ...
        gas_value,
        dest_address_bytes,
        amount_per_tx: prepared.amount_per_tx,
    };

    match sizing.mode() {
        RunMode::Sustained(plan) => {
            run_sustained_pipeline(&args, &evm, &sizing, plan, prepared.keypairs, &transfer).await
        }
        RunMode::Burst { .. } => {
            run_burst_pipeline(
                &args,
...
async fn init_solana_client_and_main_keypair(
    solana_rpc: &str,
    keypair_path: Option<&str>,
    network: Network,
) -> eyre::Result<Keypair> {
    let main_keypair = solana::load_keypair(keypair_path).await?;
    let pubkey = main_keypair.pubkey();
    let solana_rpc = solana_rpc.to_string();

    let balance = spawn_blocking(move || {
        let rpc_client = RpcClient::new_with_commitment(solana_rpc, CommitmentConfig::finalized());

        rpc_client.get_balance(&pubkey).unwrap_or(0)
    })
    .await?;
...
fn parse_gas_value(gas_value: Option<&str>, network: Network) -> eyre::Result<u64> {
...
fn resolve_evm_targets_and_receiver(
    ...
    let its_proxy_addr: Address = dest_cfg
        .contract_address(ChainContract::InterchainTokenService, dest)?
        .parse()?;
    ...
    let evm_gateway_addr: Address = dest_cfg
...
fn deployer_spl_balance(rpc_client: &RpcClient, owner: &Pubkey, mint: &Pubkey) -> u64 {
...
type SolanaTokenSetup = ([u8; 32], [u8; 32], Pubkey);
...
async fn setup_its_token(
    ...
    if let Some(token) = reusable_config_token(keypair, &request).await? {
        return Ok(token);
    }

    if let Some(token) = reusable_cached_token(keypair, &request).await? {
        return Ok(token);
    }
...
fn resolve_token_override(value: &str, network: Network) -> eyre::Result<SolanaTokenSetup> {
...
async fn reusable_config_token(
    keypair: &Keypair,
    request: &TokenSetupRequest,
    ...
    let Some((balance, needed)) = inspect_token_balance(keypair, request, mint).await? else {
        return Ok(None);
    };
...
async fn reusable_cached_token(
    ...
    let cache = read_its_cache(&request.source_chain, &request.destination_chain).await;
...
async fn inspect_token_balance(
    keypair: &Keypair,
    request: &TokenSetupRequest,
    mint: Pubkey,
) -> eyre::Result<Option<(u64, u64)>> {
    let solana_rpc = request.solana_rpc.clone();
    let owner = keypair.pubkey();
    let transfers = request.num_txs;
    ...
    spawn_blocking(move || {
        ...
        let needed = required_solana_token_balance(mint_decimals(&rpc_client, &mint), transfers);
        let balance = deployer_spl_balance(&rpc_client, &owner, &mint);
...
async fn deploy_fresh_token(
    ...
    let keypair = keypairs::clone_keypair(keypair)?;

    let deployment = spawn_blocking(move || {
        deploy_fresh_source_token(&solana_rpc, &keypair, network, dest, gas_value)
    })
    .await??;
    ...
    super::verify::wait_for_its_remote_deploy(
        &config,
        &src,
        &deployment.destination_chain,
        &deployment.message_id,
        evm_gateway_addr,
        &evm_rpc_url,
    )
    .await?;
    ...
    cache.token_id = Some(hex::encode(deployment.token_id));
    cache.salt = Some(hex::encode(deployment.salt));
    cache.mint = Some(deployment.mint.to_string());

    save_its_cache(&src, &deployment.destination_chain, &cache).await?;

    Ok((deployment.token_id, deployment.salt, deployment.mint))
}

struct FreshTokenDeployment {
    token_id: [u8; 32],
    salt: [u8; 32],
    mint: Pubkey,
    destination_chain: String,
...
fn deploy_fresh_source_token(
    solana_rpc: &str,
    keypair: &Keypair,
    network: Network,
    destination_chain: String,
    gas_value: u64,
) -> eyre::Result<FreshTokenDeployment> {
    let salt = generate_salt();
    let total_supply: u64 = 1_000_000 * 1_000_000_000;
    let spec = LOAD_TEST_SOL_SPEC;
    ...
    ui::tx_hash("deploy tx", &deploy_signature);
    ...
    ui::info(&format!(
        "deploying remote token to {destination_chain} (gas: {deploy_gas_value} lamports)..."
    ));

    let remote_signature = solana::send_its_deploy_remote_interchain_token(
        solana_rpc,
        keypair,
        network,
        &salt,
    ...
    ui::tx_hash("remote deploy tx", &remote_signature);
    ui::success("remote deploy tx confirmed on Solana");

    let message_id = solana::extract_its_message_id(solana_rpc, network, &remote_signature)
        .map_err(|error| {
            eyre!(
                "could not extract remote-deploy message ID from tx logs: {error}\n\
...
fn prepare_source_accounts(
    solana_rpc: &str,
    main_keypair: &Keypair,
    mint: Pubkey,
    sizing: RunSizing,
) -> eyre::Result<PreparedSource> {
    let rpc_client =
        RpcClient::new_with_commitment(solana_rpc.to_string(), CommitmentConfig::finalized());
    let decimals = mint_decimals(&rpc_client, &mint);
    let scale = 10u64.pow(u32::from(decimals));
    let amount_per_tx = WHOLE_TOKENS_PER_TX * scale / 100;
    let amount_per_key = WHOLE_TOKENS_PER_KEY * scale / 100;

    let keypairs = prepare_keypairs(solana_rpc, sizing.num_keys, main_keypair)?;
...
fn distribute_its_tokens(
    ...
    let source_ata = Pubkey::find_program_address(
    ...
    for (i, kp) in keypairs.iter().enumerate() {
        ...
        let dest_ata = Pubkey::find_program_address(
