...
fn ensure_cosmos_gateway(cfg: &ChainsConfig, destination: &str) -> Result<()> {
    match cfg
        .axelar
        .contract_address(AxelarChainContract::Gateway, destination)
    {
        Ok(_) => Ok(()),
        Err(_) => Err(eyre::eyre!(
            "destination chain '{destination}' has no Cosmos Gateway in the config. \
             Verification would fail. Pick a chain that has a Gateway entry, such as:\n  {}",
            list_gateway_chains(cfg).join(", ")
        )),
    }
}

#[path = "gmp/sui_destination.rs"]
mod sui_destination;
...
async fn prepare_sol_to_evm_destination(
    ...
    let mut cache = read_cache(destination).await;
    ...
    if let Some(addr_str) = cache.sender_receiver_address.as_deref() {
        ...
        let needs_redeploy = if code.is_empty() {
            ...
            let sender_receiver = SenderReceiver::new(addr, &read_provider);
        ...
        save_cache(destination, &cache).await?;
        return Ok((addr, provider));
    }
...
pub(super) async fn run_sol_to_evm(args: LoadTestArgs, sizing: RunSizing) -> Result<()> {
    ...
    if !dst_legacy {
        ensure_cosmos_gateway(&cfg, dest)?;
    }
    ...
    let gas_service_addr = cfg
        ...
        .get(dest.as_ref())
        .ok_or_else(|| eyre::eyre!("chain '{}' not found in config", dest))?
        .contract_address(ChainContract::AxelarGasService, dest)?
        .parse()?;
...
pub(super) async fn run_evm_to_sol(args: LoadTestArgs, sizing: RunSizing) -> Result<()> {
    ...
    let gateway_addr = source_config
        .contract_address(ChainContract::AxelarGateway, src)?
        .parse()?;
    let gas_service_addr = source_config
    ...
    let destination_address = memo_program_id(args.network).to_string();
        ...
        gmp_verification::run_streaming(
            ...
            |context| {
                evm_sender::run_sustained_load_test_with_metrics(evm_sender::SustainedLoadRequest {
                    ...
                    payload_encoding: GmpPayloadEncoding::SolanaExecutable,
                })
            },
        )
        .await?
...
async fn prepare_evm_gmp_destination(
    ...
    let signer_address = signer.address();
    let read_provider = ProviderBuilder::new().connect_http(rpc_url.parse()?);
    if read_provider.get_balance(signer_address).await? == U256::ZERO {
        eyre::bail!(
...
pub(super) async fn run_evm_to_evm(args: LoadTestArgs, sizing: RunSizing) -> Result<()> {
    let src = &args.source_chain;
    let dest = &args.destination_chain;
        ...
        gmp_verification::run_streaming(
            ...
            |context| {
                evm_sender::run_sustained_load_test_with_metrics(evm_sender::SustainedLoadRequest {
                    ...
                    send_done: Some(context.send_done),
                    verify_spinner_tx: context.spinner_tx,
                    payload_encoding: GmpPayloadEncoding::AbiString,
                })
            },
        )
        .await?
...
async fn prepare_cached_evm_destination(
...
) -> Result<(Address, impl Provider, Address)> {
    let destination = &args.destination_chain;
    ...
    let cache = read_cache(destination).await;
...
async fn prepare_stellar_gmp_source(
    ...
    let network_type = read_stellar_network_type(&args.config, source).await?;
    ...
    let example_contract =
        read_stellar_contract_address(&args.config, source, ChainContract::AxelarExample).await?;
    let gateway_contract =
        read_stellar_contract_address(&args.config, source, ChainContract::AxelarGateway).await?;
    let gas_token_contract = read_stellar_token_address(&args.config, source).await?;
...
pub(super) async fn run_stellar_to_evm(args: LoadTestArgs, run_sizing: RunSizing) -> Result<()> {
    let src = &args.source_chain;
    let dest = &args.destination_chain;
    let cfg = ChainsConfig::load(&args.config).await?;
...
pub(super) async fn run_evm_to_stellar(args: LoadTestArgs, sizing: RunSizing) -> Result<()> {
    ...
    let stellar_network_type = read_stellar_network_type(&args.config, dest).await?;
    let stellar_gateway_addr =
        read_stellar_contract_address(&args.config, dest, ChainContract::AxelarGateway).await?;
    let stellar_example_addr =
        read_stellar_contract_address(&args.config, dest, ChainContract::AxelarExample).await?;
    ...
    let signer_pk: [u8; 32] = keccak256(signer.address().as_slice()).into();
    gmp_verification::attach_batch(
        &args,
...
pub(super) async fn run_sol_to_stellar(args: LoadTestArgs, sizing: RunSizing) -> Result<()> {
    ...
    let signer_pk: [u8; 32] = {
        let kp = load_keypair(args.keypair.as_deref()).await?;
        let pk = SolanaSigner::pubkey(&kp);
        let mut out = [0u8; 32];
...
async fn prepare_sui_gmp_source(
    args: &LoadTestArgs,
) -> Result<(SuiClient, SuiWallet, SuiContractsConfig)> {
    let (configured_rpc, contracts) =
        read_sui_chain_config(&args.config, &args.source_chain).await?;
...
pub(super) async fn run_sui_to_evm(args: LoadTestArgs, sizing: RunSizing) -> Result<()> {
...
pub(super) async fn run_sui_to_sol(args: LoadTestArgs, sizing: RunSizing) -> Result<()> {
    ...
    let (sui_rpc, sui_contracts) = read_sui_chain_config(&args.config, src).await?;
    ...
    let burst = super::gmp_sui_source::run_sequential(
        ...
        (0..num_txs)
            .map(|_| super::gmp_sui_source::GmpSuiSubmitJob {
                payload: make_executable_payload(&inner_payload, &counter_pda),
            })
            .collect(),
    )
    .await?;
