...
async fn resolve_evm_context(
    ...
    let token_id_bytes =
        read_sui_axe_token_id(&args.config, dest, args.token_id.as_deref()).await?;
...
async fn resolve_sui_context(args: &LoadTestArgs) -> eyre::Result<SuiContext> {
    let dest = &args.destination_chain;
