...
fn resolve_config_route(
    ...
    Ok(ConfigRoute {
        ...
        destination_its: destination_config
            .contract_address("InterchainTokenService", &destination)?
            .parse()?,
        ...
        source_cosm_gateway: cfg
            .axelar
            .contract_address("Gateway", source_axelar_id.as_str())?
            .to_string(),
        voting_verifier: cfg
            .axelar
            .contract_address("VotingVerifier", source_axelar_id.as_str())?
            .to_string(),
        destination_cosm_gateway: cfg
            .axelar
            .contract_address("Gateway", destination_axelar_id.as_str())?
            .to_string(),
        destination_multisig_prover: cfg
            .axelar
            .contract_address("MultisigProver", destination_axelar_id.as_str())?
            .to_string(),
...
async fn prepare_config_signers(
    ...
    crate::cosmos::check_axelar_balance(&lcd, &chain_id, &axelar_address, &fee_denom, 200_000)
        .await?;
    let solana = crate::solana::load_keypair(None)?;
...
fn source_token_accounts(
    ...
    let source_account = solana_sdk::pubkey::Pubkey::find_program_address(
...
async fn send_config_phase_b<P: alloy::providers::Provider>(
    ...
    let (signature, _) =
        crate::solana::send_its_interchain_transfer(crate::solana::InterchainTransferRequest {
            rpc_url: &request.route.source_rpc,
            keypair: &request.signers.solana,
            network: request.network,
            token_id: &request.token_id,
            source_account: &source_account,
            mint: &mint,
            destination_chain: request.route.destination_axelar_id.as_str(),
            destination_address: &receiver,
            amount: request.amount,
            gas_value: request.gas_value,
        })?;
    ui::tx_hash("solana tx", &signature);
    let first_leg_message_id = crate::solana::extract_its_message_id(
        &request.route.source_rpc,
        request.network,
        &signature,
    )?;
    ui::kv("first-leg message_id", &first_leg_message_id);
    let gateway = crate::solana::extract_gateway_call_contract_payload(
        &request.route.source_rpc,
        &signature,
    )?;
